...
struct filter_blobs_limit_data {
	struct list_objects_filter_map *omits;
	...
	void *filter_data_)
{
	...
	case LOFT_BLOB:
		...
		if (filter_data->omits &&
		    list_objects_filter_map_contains(filter_data->omits,
						     &obj->oid)) {
			if (!is_special_filename)
				return LOFR_ZERO;

			list_objects_filter_map_remove(filter_data->omits,
						       &obj->oid);
			return LOFR_MARK_SEEN | LOFR_SHOW;
		}
	...
	unsigned long large_byte_limit)
{
	...
	if (print_omitted_object) {
		...
		list_objects_filter_map_init(d.omits, DEFAULT_MAP_SIZE);
	}
	...
	if (print_omitted_object) {
		list_objects_filter_map_foreach(d.omits, print_omitted_object,
						ctx_data);
		list_objects_filter_map_clear(d.omits);
	}
}
