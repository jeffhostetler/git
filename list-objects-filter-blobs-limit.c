	...
	void *filter_data_)
{
	...
	case LOFT_BLOB:
		...
		/*
		 * If previously provisionally omitted this blob (because
		 * of size), but it now has a special filename, force
		 * include it.
		 */
		...
		if (t == OBJ_BLOB && object_length < filter_data->max_bytes)
			return LOFR_MARK_SEEN | LOFR_SHOW;
		...
		/*
		 * We DO NOT have the blob locally, so we cannot apply the
		 * filter criteria.  Be conservative and force show it (and
		 * let the caller deal with the ambiguity).  (This matches
		 * the behavior when the special filename matches.)
		 */
		return LOFR_MARK_SEEN | LOFR_SHOW;
	}
}
