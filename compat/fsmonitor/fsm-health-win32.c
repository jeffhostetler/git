		       ...
		       BY_HANDLE_FILE_INFORMATION *bhfi)
{
	...
	if (hDir == INVALID_HANDLE_VALUE) {
		error(_("[GLE %ld] health thread could not open '%S'"),
		      GetLastError(), wpath);
		return -1;
	}

	if (!GetFileInformationByHandle(hDir, bhfi)) {
		error(_("[GLE %ld] health thread getting BHFI for '%S'"),
		      GetLastError(), wpath);
		CloseHandle(hDir);
		return -1;
	}
 ...
 * GetOverlappedResult(), it is not guaranteed.  And because
			      ...
			      enum interval_fn_ctx ctx)
{
	...
	case CTX_TIMER:
		...
		if (!bhfi_eq(&data->wt_moved.bhfi, &bhfi)) {
			error(_("BHFI changed '%S'"), data->wt_moved.wpath);
			return -1;
		}
		return 0;
