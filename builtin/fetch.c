...
static int fetch_multiple(struct string_list *list)
{
	...
	if (filter_options.choice) {
		/*
		 * We currently only support partial-fetches
		 * to the remote used for the partial-clone
		 * because we only support 1 promisor remote.
		 *
		 * Note that the loop below will spawn background
		 * fetches for each remote and one of them may
		 * INHERIT partial-fetch settings, so everything
		 * is consistent.
		 */
