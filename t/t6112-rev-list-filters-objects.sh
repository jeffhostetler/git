...
test_expect_success 'omit-all-blobs omitted 5 blobs' '
	git ls-files -s file.1 file.2 file.3 file.4 file.5 \
		| awk -f print_2.awk \
		| sort >expected &&
	git rev-list HEAD --quiet --objects --filter-print-omitted --filter=blob:none \
		| awk -f print_1.awk \
		| sed "s/~//" >observed &&
...
test_expect_success 'omit-all-blobs nothing else changed' '
	git rev-list HEAD --objects \
		| awk -f print_1.awk \
		| sort >expected &&
	git rev-list HEAD --objects --filter-print-omitted --filter=blob:none \
		| awk -f print_1.awk \
		| sed "s/~//" \
		| sort >observed &&
	test_cmp observed expected
...
test_expect_success 'omit-large-blobs omit 2 blobs' '
	git ls-files -s large.1000 large.10000 \
		| awk -f print_2.awk \
		| sort >expected &&
	git rev-list HEAD --quiet --objects --filter-print-omitted --filter=blob:limit=500 \
		| awk -f print_1.awk \
		| sed "s/~//" >observed &&
	test_cmp observed expected
'
...
test_expect_success 'omit-large-blobs nothing else changed' '
	git rev-list HEAD --objects \
		| awk -f print_1.awk \
		| sort >expected &&
	git rev-list HEAD --objects --filter-print-omitted --filter=blob:limit=500 \
		| awk -f print_1.awk \
		| sed "s/~//" \
		| sort >observed &&
	test_cmp observed expected
...
test_expect_success 'omit-large-blobs omit 2 blobs' '
	git ls-files -s large.1000 large.10000 \
		| awk -f print_2.awk \
		| sort >expected &&
	git rev-list HEAD --quiet --objects --filter-print-omitted --filter=blob:limit=1000 \
		| awk -f print_1.awk \
		| sed "s/~//" >observed &&
	test_cmp observed expected
'
...
test_expect_success 'omit-large-blobs omit 1 blob' '
	git ls-files -s large.10000 \
		| awk -f print_2.awk \
		| sort >expected &&
	git rev-list HEAD --quiet --objects --filter-print-omitted --filter=blob:limit=1001 \
		| awk -f print_1.awk \
		| sed "s/~//" >observed &&
	test_cmp observed expected
'
...
test_expect_success 'omit-large-blobs omit 1 blob (1k)' '
	git ls-files -s large.10000 \
		| awk -f print_2.awk \
		| sort >expected &&
	git rev-list HEAD --quiet --objects --filter-print-omitted --filter=blob:limit=1k \
		| awk -f print_1.awk \
		| sed "s/~//" >observed &&
	test_cmp observed expected
'
...
test_expect_success 'omit-large-blobs omit no blob (1m)' '
	...
	git rev-list HEAD --quiet --objects --filter-print-omitted --filter=blob:limit=1m \
		| awk -f print_1.awk \
		| sed "s/~//" >observed &&
	test_cmp observed expected
'
...
test_expect_success 'sparse using path pattern1' '
	git rev-list HEAD --objects --filter-print-omitted --filter=sparse:path=pattern1 >out &&
...
test_expect_success 'sparse using path pattern2' '
	git rev-list HEAD --objects --filter-print-omitted --filter=sparse:path=pattern2 >out &&
...
test_expect_success 'sparse using OID for pattern1' '
	...
	git rev-list HEAD --objects --filter-print-omitted --filter=sparse:oid=`cat pattern1.oid` >out &&
...
test_expect_success 'sparse using blob-ish to get OID for pattern spec' '
	git rev-list HEAD --objects --filter-print-omitted --filter=sparse:oid=HEAD:pattern1 >out &&
...
test_expect_success 'sparse using OID for pattern2' '
	...
	git rev-list HEAD --objects --filter-print-omitted --filter=sparse:oid=`cat pattern2.oid` >out &&
...
test_expect_success 'sparse using blob-ish rather than OID for pattern2' '
	git rev-list HEAD --objects --filter-print-omitted --filter=sparse:oid=HEAD:pattern2 >out &&
