	...
	void *filter_data_)
{
	...
	case LOFT_BLOB:
		...
		if (val > 0) {
			if (entry_prev)
				oidset2_remove(&filter_data->omits,
					       &obj->oid);
		 ...
		 * that this pathname is not in the sparse-checkout
		 * specification, so we *WANT* to omit this blob.
		 *
		 * However, a pathname elsewhere in the tree may also
		 * reference this same blob, so we cannot reject it
		 * yet.  Leave the LOFR_ bits unset so that if the
		 * blob appears again in the traversal, we will be
		 * asked again.
		 *
		 * The pathname we associate with this omit is just
		 * the first one we saw for this blob.  Other
		 * instances of this blob may have other pathnames
		 * and that is fine.  We just use it for perf
		 * because most of the time, the blob will be in
		...
		if (t == OBJ_NONE) /* we may not have it locally */
			oidset2_insert_without_length(&filter_data->omits,
						      &obj->oid, obj->type,
						      pathname);
		else
			oidset2_insert(&filter_data->omits, &obj->oid,
				       obj->type, object_length, pathname);
		frame->child_prov_omit = 1;
		return LOFR_ZERO;
	}
}
