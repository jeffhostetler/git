	...
	void *filter_data_)
{
	...
	case LOFT_BLOB:
		...
		/*
		 * Lookup the blob to get its size and confirm its type,
		 * if present.  If it is missing, we substitute -1 for
		 * the length.
		 */
		 ...
		 */
		oidset2_insert(&filter_data->omits, &obj->oid, obj->type,
			       object_length, pathname);
		obj->flags |= FILTER_REVISIT;
		frame->child_prov_omit = 1;
		return LOFR_ZERO;
	}
